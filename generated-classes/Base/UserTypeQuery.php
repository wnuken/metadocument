 ...
 * @method     ChildUserTypeQuery orderByType($order = Criteria::ASC) Order by the type column
 ...
 * @method     ChildUserTypeQuery groupByType() Group by the type column
 ...
 * @method     ChildUserType findOneByType(string $type) Return the first ChildUserType filtered by the type column
 ...
 * @method     ChildUserType requireOneByType(string $type) Return the first ChildUserType filtered by the type column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUserType[]|ObjectCollection findByType(string $type) Return ChildUserType objects filtered by the type column
...
abstract class UserTypeQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'metadocument', $modelName = '\\UserType', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = UserTypeTableMap::getInstanceFromPool((string) $key))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT id, type, description FROM user_type WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            UserTypeTableMap::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByType($type = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(UserTypeTableMap::COL_TYPE, $type, $comparison);
    }

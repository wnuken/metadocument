...
use \AdminUser as ChildAdminUser;
 ...
 * @method     ChildAdminUserQuery orderByGfolder($order = Criteria::ASC) Order by the g_folder column
 * @method     ChildAdminUserQuery orderByRol($order = Criteria::ASC) Order by the id_rol column
 *
 ...
 * @method     ChildAdminUserQuery groupByGfolder() Group by the g_folder column
 * @method     ChildAdminUserQuery groupByRol() Group by the id_rol column
 *
 ...
 * @method     ChildAdminUser findOneByGfolder(string $g_folder) Return the first ChildAdminUser filtered by the g_folder column
 * @method     ChildAdminUser findOneByRol(int $id_rol) Return the first ChildAdminUser filtered by the id_rol column *
 ...
 * @method     ChildAdminUser requireOneByGfolder(string $g_folder) Return the first ChildAdminUser filtered by the g_folder column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildAdminUser requireOneByRol(int $id_rol) Return the first ChildAdminUser filtered by the id_rol column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildAdminUser[]|ObjectCollection findByGfolder(string $g_folder) Return ChildAdminUser objects filtered by the g_folder column
 * @method     ChildAdminUser[]|ObjectCollection findByRol(int $id_rol) Return ChildAdminUser objects filtered by the id_rol column
...
abstract class AdminUserQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Base\AdminUserQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'metadocument', $modelName = '\\AdminUser', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, user, password, g_folder, id_rol FROM admin_user WHERE id = :p0';
        try {
    ...
    public function filterByUser($user = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(AdminUserTableMap::COL_USER, $user, $comparison);
    }
    ...
    /**
     * Filter the query on the g_folder column
     *
     * Example usage:
     * <code>
     * $query->filterByGfolder('fooValue');   // WHERE g_folder = 'fooValue'
     * $query->filterByGfolder('%fooValue%'); // WHERE g_folder LIKE '%fooValue%'
     * </code>
     *
     * @param     string $gfolder The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildAdminUserQuery The current query, for fluid interface
     */
    public function filterByGfolder($gfolder = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($gfolder)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $gfolder)) {
                $gfolder = str_replace('*', '%', $gfolder);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(AdminUserTableMap::COL_G_FOLDER, $gfolder, $comparison);
    }
     ...
     * Example usage:
     * <code>
     * $query->filterByRol(1234); // WHERE id_rol = 1234
     * $query->filterByRol(array(12, 34)); // WHERE id_rol IN (12, 34)
     * $query->filterByRol(array('min' => 12)); // WHERE id_rol > 12
     * </code>
     *
    ...
    public function filterByRol($rol = null, $comparison = null)
    {
        if (is_array($rol)) {
            $useMinMax = false;
            if (isset($rol['min'])) {
                $this->addUsingAlias(AdminUserTableMap::COL_ID_ROL, $rol['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($rol['max'])) {
                $this->addUsingAlias(AdminUserTableMap::COL_ID_ROL, $rol['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AdminUserTableMap::COL_ID_ROL, $rol, $comparison);
    }

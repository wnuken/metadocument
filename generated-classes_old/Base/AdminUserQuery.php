 ...
 *
 * @method     ChildAdminUserQuery leftJoinRoles($relationAlias = null) Adds a LEFT JOIN clause to the query using the Roles relation
 * @method     ChildAdminUserQuery rightJoinRoles($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Roles relation
 * @method     ChildAdminUserQuery innerJoinRoles($relationAlias = null) Adds a INNER JOIN clause to the query using the Roles relation
 *
 * @method     \RolesQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class AdminUserQuery extends ModelCriteria
{
    ...
    public function filterByRolId($rolId = null, $comparison = null)
    {
        if (is_array($rolId)) {
            $useMinMax = false;
            if (isset($rolId['min'])) {
                $this->addUsingAlias(AdminUserTableMap::COL_ROL_ID, $rolId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($rolId['max'])) {
                $this->addUsingAlias(AdminUserTableMap::COL_ROL_ID, $rolId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AdminUserTableMap::COL_ROL_ID, $rolId, $comparison);
    }
    ...
    public function filterByRoles($roles, $comparison = null)
    {
        if ($roles instanceof \Roles) {
            return $this
                ->addUsingAlias(AdminUserTableMap::COL_ROL_ID, $roles->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(AdminUserTableMap::COL_ROL_ID, $roles->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
